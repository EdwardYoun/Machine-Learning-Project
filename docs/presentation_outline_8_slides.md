...
- Kaggle / Big Data Bowl limitation:
  - we had access to weekly `2023` train tracking files
  - the provided `2024` holdout tracking data was much smaller and more limited
- Split design:
...
- Methodology adjustment from the initial plan:
  - because the Kaggle tracking release gave us strong `2023` train coverage but only a limited `2024` test slice,
  - we revised the original setup to use rolling validation inside `2023` and reserve `2024` as the final holdout
- Selection metrics:
...
- Final methodology note:
  - revising the original split design was necessary to make the project academically defensible under the Kaggle data constraints
- Final conclusions:
